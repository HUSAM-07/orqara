...
test("preserves a blockquoted notice ahead of the bullets", () => {
  const contents = formatFdroidChangelog([
    ...
    ">",
    "> If you installed Orqara Desktop 0.1.108, you need to [reinstall manually](https://orqara.pages.dev/download).",
    "",
    "### Fixed",
    "",
  ...
  assert.match(
    contents,
    /^Important update notice\nIf you installed Orqara Desktop 0\.1\.108, you need to reinstall manually\.\n\nFixed\n- Desktop no longer gets stuck connecting\./,
  );
  assert.ok(contents.length <= CHANGELOG_CHARACTER_LIMIT);
});
...
test("keeps the notice whole and cuts bullets when the budget runs out", () => {
  ...
  const contents = formatFdroidChangelog([
    "> Reinstall Orqara manually before updating, the old updater cannot install this build.",
    "",
    "### Fixed",
    "",
    ...bullets,
...
test("renders the real 0.1.109 notice, which is the entry's whole point", () => {
  const changelog = [
    ...
    "> **Important update notice**",
    ">",
    "> If you installed Orqara Desktop 0.1.108, you need to [download and reinstall Orqara manually](https://orqara.pages.dev/download) to get this fix. The bug in 0.1.108 prevents its automatic updater from installing 0.1.109. Users on 0.1.107 or earlier can update normally.",
    "",
    "### Fixed",
    "",
    "- Orqara Desktop no longer gets stuck connecting or loses native window controls after updating ([#2111](https://github.com/HUSAM-07/orqara/pull/2111) by [@cleiter](https://github.com/cleiter))",
    "",
  ].join("\n");
  ...
  withTempRepo(
    (dir) => {
      ...
      assert.match(contents, /download and reinstall Orqara manually/);
      assert.match(contents, /Users on 0\.1\.107 or earlier can update normally\./);
      assert.match(contents, /\nFixed\n- Orqara Desktop no longer gets stuck connecting/);
