...
buildNpmPackage {
  ...
  installPhase = ''
    ...
    ${lib.optionalString stdenv.hostPlatform.isDarwin ''
      app="$(find packages/desktop/release -maxdepth 3 -type d -name Orqara.app -print -quit)"
      if [ -z "$app" ]; then
        echo "electron-builder did not produce Orqara.app" >&2
        exit 1
      fi
      mkdir -p "$out/Applications"
      cp -R "$app" "$out/Applications/Orqara.app"
      ln -s ../Applications/Orqara.app/Contents/MacOS/Orqara "$out/bin/paseo-desktop"
    ''}
  ...
  desktopItems = lib.optionals stdenv.hostPlatform.isLinux [
    ...
    # main.ts sets ("Orqara"). Ship a NoDisplay entry for the second spelling so
    ...
    (makeDesktopItem {
      ...
      icon = "paseo-desktop";
      categories = [ "Development" ];
      startupWMClass = "Orqara";
      noDisplay = true;
    })
  ];

  meta = {
    description = "Orqara desktop app (Electron wrapper)";
    homepage = "https://github.com/HUSAM-07/orqara";
