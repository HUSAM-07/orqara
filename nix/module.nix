...
in
{
  ...
  options.services.paseo = {
    enable = lib.mkEnableOption "Orqara, a self-hosted daemon for AI coding agents";
    ...
    user = lib.mkOption {
      type = lib.types.str;
      default = "paseo";
      description = "User account under which Orqara runs.";
    };

    group = lib.mkOption {
      type = lib.types.str;
      default = "paseo";
      description = "Group under which Orqara runs.";
    };
    ...
    dataDir = lib.mkOption {
      ...
      '';
      description = "Directory for Orqara state (PASEO_HOME). Stores agent data, config, and logs.";
    };

    port = lib.mkOption {
      type = lib.types.port;
      default = 6767;
      description = "Port for the Orqara daemon to listen on.";
    };

    listenAddress = lib.mkOption {
      type = lib.types.str;
      default = "127.0.0.1";
      description = "Address for the Orqara daemon to bind to.";
    };

    openFirewall = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to open the firewall for the Orqara daemon port.";
    };
    ...
    hostnames = lib.mkOption {
      ...
      description = ''
        Hostnames the Orqara daemon accepts in the Host header (DNS rebinding protection).
    ...
    inheritUserEnvironment = lib.mkOption {
      ...
      description = ''
        ...
        When Orqara runs as a real user (not the default system user), AI agents
    ...
    environment = lib.mkOption {
      ...
      '';
      description = "Extra environment variables for the Orqara daemon.";
    };
  ...
  config = lib.mkIf cfg.enable (
    ...
    systemd.services.paseo = {
      description = "Orqara - self-hosted daemon for AI coding agents";
