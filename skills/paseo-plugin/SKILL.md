---
name: paseo-plugin
description: Build and manage trusted local Orqara plugins. Use when the user asks to create, edit, install, reload, enable, disable, remove, or troubleshoot a Orqara plugin; add lifecycle hooks; transform agent configuration, environment, MCP servers, or workspace creation; automate permissions or turn follow-ups; add a native surface, sidebar item, or workspace panel; add Command Center items or slash commands; add composer pills or attachment sources; transform, render, or append agent timeline items; contribute a theme; use Orqara from plugin code; or add plugin RPCs.
---
...
Fetch [https://orqara.pages.dev/llms.txt](https://orqara.pages.dev/llms.txt) first. Select and fetch the current plugin Markdown pages from that index before changing a plugin:

- [Plugin quickstart](https://orqara.pages.dev/docs/plugins.md) ([browser page](https://orqara.pages.dev/docs/plugins))
- [Plugin reference](https://orqara.pages.dev/docs/plugins/v0.8/reference.md) ([browser page](https://orqara.pages.dev/docs/plugins/v0.8/reference))
...
In the Orqara repository, use `public-docs/plugins/v0.8/reference.md` for the checkout's API, including
...
| Plugin RPC                | `defineRpc` + `server.handle` + `useRpc`         | Daemon-side work that is not a normal Orqara operation: vendor APIs, credentials, local files                 | reference.md → Add plugin-specific backend behavior                                                |
| Lifecycle events          | `server.on`                                      | Observe agent/workspace lifecycle, inspect ended turns, and answer permission requests                        | [Lifecycle hooks](https://orqara.pages.dev/docs/plugins/v0.8/reference.md#lifecycle-hooks)         |
| Creation and launch hooks | `server.before`                                  | Change agent config, provider options, MCP servers, environment, or workspace isolation before the operation  | [Before hooks](https://orqara.pages.dev/docs/plugins/v0.8/reference.md#before-hooks)               |
| Orqara SDK                | `usePaseo()` / handler `{ paseo }`               | Normal Orqara operations: workspaces, agents, providers, config                                               | reference.md → Use the Orqara SDK                                                                  |

| Lifecycle task                                                      | Example                                                                                                 |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Log all eleven hooks                                                | [lifecycle-logger](https://github.com/HUSAM-07/orqara/tree/main/plugin-examples/lifecycle-logger)       |
| Follow-ups, permissions, environment, provider switching, worktrees | [lifecycle-actions](https://github.com/HUSAM-07/orqara/tree/main/plugin-examples/lifecycle-actions)     |
| Inject MCP servers and change Codex sandbox/approval options        | [agent-configuration](https://github.com/HUSAM-07/orqara/tree/main/plugin-examples/agent-configuration) |
...
The manifest supplies the default install ID and supported Orqara versions:
...
Orqara release is incompatible. Use npm semver ranges and explicitly include beta versions when
...
See [requirements](https://orqara.pages.dev/docs/plugins/v0.8/reference#requirements).
...
created by plugin code. Every client `add*` method returns an idempotent remover. Orqara calls the
...
mobile, and Orqara has multiple themes. Every `Text` must take its color from `theme.colors`.
...
Icons are Lucide icon names. `theme` is a typed `PluginTheme` on every surface and panel. Primary text uses `theme.colors.foreground`; labels use `theme.colors.foregroundMuted`; the root view uses `theme.colors.surface0`. `layout.compact` is true on mobile and narrow windows. Orqara owns the route, header, host picker, close action, error boundary, and per-installation query client.

Client code may import `react`, `react-native`, `@tanstack/react-query`, `zod`, `@getpaseo/plugin`, and `@getpaseo/plugin/react-native`. Install dependencies locally for typechecking; Orqara supplies these runtime modules. JSX compiles with the automatic runtime, so no `React` import is needed for JSX. Importing a `node:` module from client code is a compile error.

| Module                          | Use it for                                                                                               |
| ------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `@getpaseo/plugin`              | contribution contracts, shared definitions, RPC input/output types, `usePaseo`, `useRpc`, and data hooks |
| `@getpaseo/plugin/react-native` | Orqara UI: `Icon`, `Modal`, `useToast`, `useRevealedText`                                                |
...
Use the existing Orqara SDK for normal Orqara operations. Use plugin RPC only for plugin-specific backend behavior.

### Call Orqara from a surface
...
The API covers workspaces, agents, providers, and daemon config. It omits connection lifecycle because Orqara owns the connection. Consult the current [SDK reference](https://orqara.pages.dev/docs/sdk/reference.md) for method details.
...
`console.error()` writes to stderr. Orqara captures both streams without interfering with plugin IPC.
...
Return complete text snapshots. Orqara owns the composer menu, picker, pills, drafts, and submission. Credentials and vendor calls stay in the daemon handler.
...
Call `contributeClient(client)` from `index.client.tsx`, or move its body into that entry. The component owns its icon and text; Orqara owns the pressable, chrome, pending state, error reporting, and placement. Removal functions are idempotent, and Orqara removes every pill when the plugin, client entrypoint, or host connection is torn down.
...
`addTheme` takes a small light or dark palette; Orqara expands it into the full token set. Every color is a hex string.
...
Plugins are installed per daemon and are trusted, unsandboxed code. Backend code can access files, processes, credentials, and network services on the daemon machine. Client contributions run inside the Orqara app. Do not install a plugin the user has not authorized or source code you have not inspected.
...
> Plugins are trusted, unsandboxed code. Backend plugin code can access your daemon machine, including files, processes, credentials, and network services. Client plugin code runs inside the Orqara app. May I enable plugins on this daemon?
...
When the same sidebar contribution exists on several connected hosts, Orqara shows it once with a host picker. The selected host owns the bundle, SDK calls, RPCs, and query cache. An offline selected host does not fall through to another host. Attachment sources stay scoped to the composer's host.
...
Use `--host <url>` when managing a daemon other than the CLI default. A Git source that must install or generate something declares `build` in `paseo-plugin.json` as a list of argv arrays; Orqara runs them without a shell on install and update and keeps the old version if one fails. Plugin source edits require `paseo plugin reload`; config changes to the global switch require `paseo reload`. A failed plugin reload stays failed; inspect `paseo plugin ls` for the load error and `paseo plugin logs <id>` for subprocess output, fix the source, typecheck, and reload again. `remove` deletes configuration, never the source directory.

Do not restart the daemon to load source changes. Restarting it can kill the agent performing the work.

For an old mixed entry, follow the standalone [v0.8 runtime-entry migration guide](https://orqara.pages.dev/docs/plugins/v0.8/migration) mechanically.
