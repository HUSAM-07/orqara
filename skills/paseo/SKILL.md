---
name: paseo
description: Orqara reference for managing projects, workspaces, workspace scripts, agents, schedules, and heartbeats.
---

Orqara is a remote daemon that manages coding agents, terminals. Control it through MCP tools or the CLI.
...
**`archive_workspace`** — `{ workspaceId }`. Archives the workspace, its agents, and its terminals. Local directories remain; Orqara removes an owned worktree only after its final active workspace reference is archived.
...
**`start_workspace_script`** — `{ workspaceId, scriptName }`. Starts one configured script through Orqara's managed workspace-script launcher and returns its status metadata.
