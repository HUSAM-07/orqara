...
describe("open-project-routing", () => {
  it("returns a bare absolute path argument", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
  ...
  it("does not treat flags as project paths", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara", "--version", flagLikeDirectory],
        isDefaultApp: false,
      }),
  ...
  it("returns the path from an explicit --open-project flag for backward compatibility", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara", "--open-project", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
