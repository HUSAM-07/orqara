...
describe("browser webview window-open requests", () => {
  it("routes foreground tabs to a Orqara workspace tab", () => {
  ...
  it.each(["noopener", "noreferrer"])(
    "routes a named target with %s to a Orqara workspace tab",
    (features) => {
      const result = decideBrowserWindowOpenRequest({
        url: "https://example.com/target",
        disposition: "new-window",
  ...
  it("routes Shift-clicked links to a Orqara workspace tab", () => {
    const result = decideBrowserWindowOpenRequest({
      url: "https://example.com/target",
      disposition: "new-window",
      frameName: "",
  ...
  it.each(["noopener", "noreferrer", "attributionsrc=https://example.com/register", "popup=false"])(
    "routes non-popup feature %s to a Orqara workspace tab",
    (features) => {
      const result = decideBrowserWindowOpenRequest({
        url: "https://example.com/target",
        disposition: "new-window",
  ...
  it("routes an all-enabled browser-chrome request to a Orqara workspace tab", () => {
    const result = decideBrowserWindowOpenRequest({
      url: "https://example.com/target",
      disposition: "new-window",
      frameName: "_blank",
