...
describe("passthrough CLI", () => {
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Orqara", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
  ...
  it("returns null when only --no-sandbox is present", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Orqara", "--no-sandbox"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores Electron remote debugging switches", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Orqara", "--remote-debugging-port=9233"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("preserves CLI flags for direct app invocations", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara", "--version"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["--version"]);
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Orqara.app/Contents/MacOS/Orqara", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
  ...
  it("parses terminal args for direct app CLI passthrough", () => {
    ...
    expect(
      parsePassthroughCliArgsFromArgv([
        "/Applications/Orqara.app/Contents/MacOS/Orqara",
        "daemon",
        "set-password",
      ]),
