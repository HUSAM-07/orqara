...
export function assertPluginCompatibility(input: PluginCompatibilityInput): void {
  ...
  if (!input.version || !valid(input.version)) {
    throw new Error(
      `Cannot check plugin "${input.id}" requirements: Orqara ${input.runtime} version is unknown. Update the ${input.runtime}.`,
    );
  }
  if (satisfies(input.version, range)) return;
  const action =
    input.requirements?.paseo === undefined
      ? "This plugin has no requirements.paseo and targets Orqara before 0.8. Ask its author to migrate it: https://orqara.pages.dev/docs/plugins/v0.8/migration"
      : `Use a compatible plugin version or update the ${input.runtime}.`;
  throw new Error(
    `Plugin "${input.id}" requires Orqara ${range}. Your ${input.runtime} is ${input.version}. ${action}`,
  );
}
