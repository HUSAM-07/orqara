...
export const PaseoWorktreeArchiveRequestSchema = z.object({
  ...
  // Orqara-owned. Omitted/unknown values default to "workspace" for old-client safety.
...
export const TaskEvidenceSchema = z.object({
  commandId: z.string(),
  taskId: z.string(),
  operationState: z.enum(["running", "finished"]),
  command: z.string(),
  startedAt: z.string(),
  durationMs: z.number().int().nonnegative().nullable(),
  status: z.enum(["pending", "passed", "failed", "timed_out", "stale"]),
  exitCode: z.number().int().nullable(),
  stdout: z.string(),
  stderr: z.string(),
  isolation: z.enum(["live", "disposable_checkout"]).optional().default("live"),
...
export const TaskPayloadSchema = z.object({
  ...
  check: TaskCheckSchema,
  createdAt: z.string(),
  latestEvidence: TaskEvidenceSchema.nullable(),
});
...
export const TaskCreateRequestSchema = z.object({
  ...
  checks: z.array(TaskCheckSchema).length(1),
...
export const TaskVerifyRequestSchema = z.object({
  type: z.literal("task.verify.request"),
  workspaceId: z.string().trim().min(1),
...
export const TaskEvidenceRequestSchema = z.object({
  type: z.literal("task.evidence.request"),
  workspaceId: z.string().trim().min(1),
  taskId: z.string().trim().min(1),
  requestId: z.string(),
});
...
export const TaskCommandRequestSchema = z.object({
  ...
  action: z.enum(["start", "accept", "retry", "cancel"]),
  requestId: z.string(),
});
...
export const SessionInboundMessageSchema = z.discriminatedUnion("type", [
  ...
  TaskVerifyRequestSchema,
  TaskEvidenceRequestSchema,
...
export const WorkspaceDescriptorPayloadSchema = z
  .object({
    ...
    // Present only for Orqara-owned worktrees; this is the basename of their root directory.
    worktreeSlug: z.string().optional(),
...
const TaskOperationPayloadSchema = z.object({
  requestId: z.string(),
  workspaceId: z.string(),
  task: TaskPayloadSchema.nullable(),
  evidence: TaskEvidenceSchema.nullable().optional(),
  duplicate: z.boolean().optional(),
  error: z.string().nullable(),
});

export const TaskCreateResponseMessageSchema = z.object({
  type: z.literal("task.create.response"),
  payload: TaskOperationPayloadSchema,
});

export const TaskListResponseMessageSchema = z.object({
...
export const TaskVerifyResponseMessageSchema = z.object({
  type: z.literal("task.verify.response"),
  payload: TaskOperationPayloadSchema,
});

export const TaskEvidenceResponseMessageSchema = z.object({
  type: z.literal("task.evidence.response"),
  payload: z.object({
    requestId: z.string(),
    workspaceId: z.string(),
    taskId: z.string(),
...
export const TaskCommandResponseMessageSchema = z.object({
...
export const SessionOutboundMessageSchema = z.discriminatedUnion("type", [
  ...
  TaskCreateResponseMessageSchema,
  TaskListResponseMessageSchema,
  TaskVerifyResponseMessageSchema,
  TaskEvidenceResponseMessageSchema,
  TaskCommandResponseMessageSchema,
...
export type TaskVerifyRequest = z.infer<typeof TaskVerifyRequestSchema>;
export type TaskEvidenceRequest = z.infer<typeof TaskEvidenceRequestSchema>;
export type TaskCommandRequest = z.infer<typeof TaskCommandRequestSchema>;
export type TaskCreateResponseMessage = z.infer<typeof TaskCreateResponseMessageSchema>;
export type TaskListResponseMessage = z.infer<typeof TaskListResponseMessageSchema>;
export type TaskVerifyResponseMessage = z.infer<typeof TaskVerifyResponseMessageSchema>;
export type TaskEvidenceResponseMessage = z.infer<typeof TaskEvidenceResponseMessageSchema>;
export type TaskCommandResponseMessage = z.infer<typeof TaskCommandResponseMessageSchema>;
