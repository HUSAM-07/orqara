...
test("Hub MCP configuration reaches the provider alongside Orqara MCP without entering snapshots", async () => {
...
test("new Hub executions cannot override the daemon-owned Orqara MCP server", async () => {
  const hub = await launchRelationship();
...
test("reserved Orqara MCP input does not invalidate replay of an owned execution", async () => {
  const hub = await launchRelationship();
