...
export default function contribute(server: { registerProvider(provider: ProviderRegistration): void }) {
  ...
  it("explains that an index.ts plugin was made for an older Orqara version", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "paseo-plugin-"));
    temporaryDirectories.push(directory);
    await Promise.all([
    ...
    await expect(runtime.startPlugin("legacy", directory)).rejects.toThrow(
      "This plugin was made for an older version of Orqara and cannot run on Orqara v0.8. Ask its author to update it. Plugin authors can follow the migration guide: https://orqara.pages.dev/docs/plugins/v0.8/migration",
    );
  });
...
export default function contribute(server: any) {
  ...
  return () => undefined;
    ...
    await expect(runtime.invoke("hello", "greet", { name: "Orqara" })).resolves.toMatchObject({
      message: "Hello, Orqara",
    });
