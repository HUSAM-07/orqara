...
export async function createPaseoWorktreeCommand<Result extends CreatePaseoWorktreeResult>(
  ...
  try {
    if (!dependencies.createPaseoWorktreeWorkflow) {
      throw new Error("Orqara worktree service is not configured");
    }
  ...
  if (scope === "worktree") {
    if (!ownership.allowed) {
      return {
        ok: false,
        code: "NOT_ALLOWED",
        message: "Worktree is not a Orqara-owned worktree",
        removedAgents: [],
      };
    }
...
async function resolveArchiveTarget(
  ...
  if (repoRoot && input.branchName) {
    ...
    if (!match) {
      throw new Error(`Orqara worktree not found for branch ${input.branchName}`);
    }
    return match.path;
  }
