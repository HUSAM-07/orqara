...
test("maps a nested cwd from an existing Orqara worktree into the next worktree", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const paseoHome = path.join(tempDir, ".paseo");
...
// Worktree restore (Unit 3): recreate a deleted Orqara-owned worktree from its
