...
export class OpenCodeBridge {
  ...
  private async route(request: IncomingMessage, response: ServerResponse): Promise<void> {
    try {
      ...
      if (request.method === "GET" && contextMatch) {
        ...
        if (!binding) {
          sendJson(response, 404, { error: "OpenCode session is not bound to a Orqara agent" });
          return;
        }
  ...
  private async executeTool(input: {
    ...
    if (!binding) {
      sendJson(input.response, 404, { error: "OpenCode session is not bound to a Orqara agent" });
      return;
    }
    if (!binding.tools) {
      sendJson(input.response, 403, { error: "Orqara tools are disabled for this session" });
      return;
    }
