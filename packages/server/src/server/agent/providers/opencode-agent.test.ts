...
describe("OpenCodeAgentClient adapter smoke tests", () => {
  ...
  test("single turn completes with streaming deltas", async () => {
    ...
    // own default agent instead of Orqara assuming any particular agent exists.
  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    openCodeClient.appAgentsResponse = {
      data: [
        {
          name: "orqara-test-custom",
          description: "Custom agent defined for Orqara integration test",
          mode: "primary",
        },
    ...
    expect(modes.map((mode) => mode.id)).toEqual(["orqara-test-custom"]);

    const custom = modes.find((mode) => mode.id === "orqara-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Orqara-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Orqara integration test");
    ...
    const parts = __openCodeInternals.buildOpenCodePromptParts([
      {
        ...
        url: "https://github.com/HUSAM-07/orqara/issues/55",
        body: "Issue body",
      },
    ]);
...
describe("OpenCode provider subagent contract", () => {
  ...
  test("keeps an autonomous turn active until OpenCode reaches its terminal event", async () => {
    ...
    try {
      ...
      await expect(parent.startTurn("Continue from Orqara")).rejects.toThrow(
  ...
  test("does not adopt late output from an interrupted Orqara turn", async () => {
    ...
    try {
      await parent.startTurn("Start from Orqara");
