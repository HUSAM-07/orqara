...
test("real OpenCode server shares one process while shell.env stays session-scoped", async () => {
  ...
  try {
    ...
    const callerResult = await first.run(
      [
        "Use the paseo_report_caller_agent_id tool to read your Orqara caller agent ID.",
...
function createCallerCatalog(callerAgentId: string): PaseoToolCatalog {
  const tool = {
    ...
    title: "Report Orqara caller agent ID",
    description: "Returns the caller agent ID assigned by Orqara.",
    inputSchema: {},
    async handler() {
