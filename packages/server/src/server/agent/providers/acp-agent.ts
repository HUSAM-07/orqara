...
// `available_commands_update` session update) translate that payload into Orqara
      ...
      return await this.runACPRequest(() =>
        Promise.race([
          transport.connection.initialize({
            ...
            clientInfo: { name: "Orqara", version: "dev" },
          }),
...
export class ACPAgentSession implements AgentSession, ACPClient {
  ...
  async requestPermission(params: RequestPermissionRequest): Promise<RequestPermissionResponse> {
    ...
    // Match Zed acp.rs:3189-3220 when Orqara is not handling the request locally.
