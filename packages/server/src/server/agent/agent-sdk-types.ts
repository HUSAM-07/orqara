...
export interface AgentLaunchContext {
  ...
  env?: Record<string, string>;
  /**
   * Runtime-only internal Orqara tools. This must never be persisted into
...
export interface AgentClient {
   ...
   * Called when Orqara archives an agent so the provider's own UI reflects the same state.
   */
  archiveNativeSession?(handle: AgentPersistenceHandle): Promise<void>;
  /**
   * Unarchive a durable native session in the provider.
   * Called before Orqara clears its archived flag so provider resume can succeed.
   */
