...
describe("daemon E2E (real OMP)", () => {
  ...
  test(
    "native Orqara host tools execute through RPC-UI",
    async () => {
      const harness = await createHarness();
      try {
        ...
        const items = await promptAndFinish(
          harness,
          agent.id,
          "Use the Orqara host tool list_agents exactly once. Find the agent titled native-host-tool in the result, then reply exactly HOST_TOOL_OK:<its id>.",
        );
