...
import {
  ...
  type TaskVerifyRequest,
  type TaskEvidenceRequest,
...
export class Session {
    ...
    this.unsubscribeAgentEvents = this.agentManager.subscribe(
      (event) => {
        ...
        if (event.event.type === "turn_completed") {
          void this.verifyFinishedTask(event.agentId, event.event.turnId);
        }
  ...
  private dispatchTaskMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      ...
      case "task.verify.request":
        return this.handleTaskVerifyRequest(msg);
      case "task.evidence.request":
        return this.handleTaskEvidenceRequest(msg);
  ...
  private async handleTaskListRequest(request: TaskListRequest): Promise<void> {
    try {
      this.emit({
  ...
  private async handleTaskVerifyRequest(request: TaskVerifyRequest): Promise<void> {
    try {
      ...
      const agent = task.agentId
        ...
        : null;
      if (task.agentId && !agent) throw new Error(`Task agent not found: ${task.agentId}`);
  ...
  private async handleTaskEvidenceRequest(request: TaskEvidenceRequest): Promise<void> {
    try {
      this.emit({
        type: "task.evidence.response",
        payload: {
          requestId: request.requestId,
          workspaceId: request.workspaceId,
      ...
      this.emit({
        ...
        payload: {
          ...
          taskId: request.taskId,
          evidence: [],
          error: error instanceof Error ? error.message : "Failed to load task evidence",
        },
      });
    }
  }

  private async verifyFinishedTask(agentId: string, turnId?: string): Promise<void> {
    try {
      ...
      this.sessionLogger.warn({ err: error, agentId }, "Failed to verify finished task");
    }
  }
    ...
    try {
      ...
      if (request.action === "start" && task.status === "queued") {
        if (task.revision !== request.expectedRevision) {
          throw new Error(
            `Revision conflict: expected ${request.expectedRevision}, current ${task.revision}`,
          );
        }
        ...
        const agentRequest: CreateAgentRequestMessage = {
          type: "create_agent_request",
          config: { provider: task.providerId, cwd: workspace.cwd },
          workspaceId: request.workspaceId,
          worktree: {
            mode: "branch-off",
            newBranch: `orqara-task-${task.id}-${task.revision}`,
          },
          initialPrompt: task.prompt,
          clientMessageId: `task:${task.id}:${request.commandId}`,
          attachments: [],
          labels: {
            "orqara.taskId": task.id,
            "orqara.taskWorkspaceId": request.workspaceId,
          },
          requestId: request.requestId,
        };
        const agentId = await this.agentRequests.create({
          key: `orqara-task:${request.workspaceId}:${request.taskId}:${request.commandId}`,
          request: {
            taskId: task.id,
            workspaceId: request.workspaceId,
