...
test("DaemonClient starts, auto-verifies, and accepts one idempotent task", async () => {
  const cwd = tmpCwd();
  writeFileSync(path.join(cwd, "file.txt"), "initial\n");
  ...
  try {
    await client.connect();
    expect(client.getLastServerInfoMessage()?.features?.tasksV1).toBe(true);
    const agent = await client.createAgent({ provider: "codex", cwd });
    expect(agent.workspaceId).toBeTruthy();
    const workspaceId = agent.workspaceId!;
    const input = {
      workspaceId,
      title: "Verify the task protocol",
      prompt: "Exercise the real daemon path.",
      providerId: "codex",
      acceptanceCriteria: ["required check passes"],
      checks: [
        {
          id: "test",
    ...
    const duplicate = await client.createTask(input);
    expect(created.task).not.toBeNull();
    expect(duplicate).toMatchObject({ duplicate: true, task: { id: created.task?.id } });
    expect((await client.listTasks(workspaceId)).tasks).toHaveLength(1);

    const started = await client.commandTask({
      workspaceId,
      taskId: created.task!.id,
      commandId: "start-task-1",
      expectedRevision: created.task!.revision,
      action: "start",
    });
    expect(started.task).toMatchObject({ status: "running", agentId: expect.any(String) });
    const duplicateStart = await client.commandTask({
      workspaceId,
      taskId: created.task!.id,
      commandId: "start-task-1",
      expectedRevision: created.task!.revision,
    ...
    expect(duplicateStart).toMatchObject({
      duplicate: true,
      task: { agentId: started.task?.agentId },
    });
    const agents = (await client.fetchAgents()).entries;
    expect(agents).toHaveLength(2);
    const taskAgent = agents.find((entry) => entry.agent.id === started.task?.agentId)?.agent;
    expect(taskAgent?.cwd).not.toBe(cwd);
    expect(taskAgent?.workspaceId).not.toBe(workspaceId);
    expect(execFileSync("git", ["status", "--porcelain"], { cwd, encoding: "utf8" })).toBe("");

    await expect
      .poll(async () => (await client.listTasks(workspaceId)).tasks[0]?.status)
      .toBe("review_ready");
    const verified = (await client.listTasks(workspaceId)).tasks[0]!;
    expect(
      (await client.getTaskEvidence({ workspaceId, taskId: created.task!.id })).evidence,
    ).toHaveLength(1);

    const accepted = await client.commandTask({
      workspaceId,
      taskId: created.task!.id,
      commandId: "accept-task-1",
