...
function candidate(input: {
  ...
  return {
    ...
    project: {
      projectKey: "key",
      projectName: input.projectName ?? "HUSAM-07/orqara",
...
describe("scoreAgentHistoryCandidate", () => {
  ...
  it("matches the project name", () => {
    expect(
      scoreAgentHistoryCandidate("orqara", candidate({ projectName: "HUSAM-07/orqara" })),
    ).not.toBeNull();
  });
...
describe("rankAgentHistoryCandidates", () => {
  ...
  it("ranks the workspace name above the project name that every session shares", () => {
    const ranked = rankAgentHistoryCandidates(
      "orqara",
      [
        candidate({ title: "unrelated work", projectName: "HUSAM-07/orqara" }),
        candidate({ workspaceName: "orqara", projectName: "HUSAM-07/orqara" }),
      ],
      byUpdatedAtDesc,
    );
    expect(ranked[0].candidate.project.workspaceName).toBe("orqara");
  });
