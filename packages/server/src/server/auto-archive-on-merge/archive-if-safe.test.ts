...
function createGitRepo(): { tempDir: string; repoDir: string } {
  ...
  execFileSync("git", ["config", "user.name", "Orqara Test"], {
    cwd: repoDir,
    stdio: "pipe",
  });
...
describe("archiveIfSafe", () => {
  ...
  test("does nothing when the cwd is not a Orqara-owned worktree", async () => {
    const harness = createHarness({
  ...
  test("archives a clean Orqara-owned worktree after merge", async () => {
    const harness = createHarness();
