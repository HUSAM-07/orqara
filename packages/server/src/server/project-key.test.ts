...
describe("deriveProjectKey", () => {
  ...
  test.each([
    "https://github.com/HUSAM-07/orqara.git",
    "ssh://git@github.com/HUSAM-07/orqara.git",
    "git@github.com:HUSAM-07/orqara.git",
  ])("normalizes common remote form %s", (remoteUrl) => {
    expect(derive(remoteUrl)).toBe("remote:github.com/husam-07/orqara");
  });

  test("normalizes GitHub casing", () => {
    expect(derive("git@github.com:HUSAM-07/orqara.git")).toBe("remote:github.com/husam-07/orqara");
  });
  ...
  test("groups an SSH remote whether or not its default port is spelled out", () => {
    expect(derive("ssh://git@github.com:22/HUSAM-07/orqara.git")).toBe(
      derive("ssh://git@github.com/HUSAM-07/orqara.git"),
    );
  });
  ...
  test("keeps a repository root distinct from one of its subprojects", () => {
    ...
    const remoteUrl = "git@github.com:HUSAM-07/orqara.git";

    expect(
      deriveProjectKey({ rootPath: worktreeRoot, remoteUrl, worktreeRoot, mainRepoRoot: null }),
    ).toBe("remote:github.com/husam-07/orqara");
    expect(
      deriveProjectKey({
    ...
    ).toBe("remote:github.com/husam-07/orqara#subdir:packages/app");
  });
  ...
  test("uses the owner and repository from a remote", () => {
    expect(
      deriveProjectGroupingDisplayName({
        ...
        remoteUrl: "git@github.com:HUSAM-07/orqara.git",
