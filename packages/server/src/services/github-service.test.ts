...
describe("ForgeService", () => {
      ...
      pullRequestTimelineJson({
        ...
        comments: {
          nodes: [
            {
              id: "IC_badge",
              body: "![build](https://img.shields.io/github/actions/workflow/status/HUSAM-07/orqara/ci.yml)",
              bodyHTML:
  ...
  it("loads GitHub merge, auto-merge, permission, policy, and queue facts for PR 993 shape", async () => {
    const runner = createScriptedRunner([
      currentPullRequestJson({
        number: 993,
        url: "https://github.com/HUSAM-07/orqara/pull/993",
        title: "Auto-merge UX",
        ...
        statusCheckRollup: [
          {
            ...
            status: "IN_PROGRESS",
            conclusion: null,
            detailsUrl: "https://github.com/HUSAM-07/orqara/actions/runs/993",
          },
        ],
      }),
      currentPullRequestGithubFactsJson(),
      ...
      checks: [
        {
          name: "server tests",
          status: "pending",
          url: "https://github.com/HUSAM-07/orqara/actions/runs/993",
          workflow: "CI",
        },
      ],
  ...
  it("treats a GitHub issue or PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.com/HUSAM-07/orqara/pull/793",
      limit: 5,
    });
  ...
  it("does not treat an unrelated tracker URL as a GitHub issue/PR number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://gitlab.com/HUSAM-07/orqara/issues/793",
      limit: 5,
    });
    ...
    expect(runner.calls.map((call) => call.args)).toEqual([
      [
        ...
        "list",
        "--search",
        "https://gitlab.com/HUSAM-07/orqara/issues/793",
        "--json",
        "number,title,url,state,body,labels,updatedAt",
        "--limit",
        "5",
  ...
  it("treats a GitHub Enterprise issue/PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.acme.internal/HUSAM-07/orqara/pull/793",
      limit: 5,
    });
