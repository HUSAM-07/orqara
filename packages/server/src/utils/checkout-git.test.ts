...
function createGitHubServiceForStatus(
  ...
  return {
    ...
    getPullRequest: async () => ({
      number: 1,
      title: "PR",
      url: "https://github.com/HUSAM-07/orqara/pull/1",
      state: "OPEN",
      body: null,
      baseRefName: "main",
      headRefName: "feature",
...
function createPullRequestStatus(overrides?: Partial<CurrentPullRequestStatus>) {
  return {
    url: "https://github.com/HUSAM-07/orqara/pull/123",
    title: "Ship feature",
    state: "open",
    baseRefName: "main",
    headRefName: "feature",
...
`;
  ...
  it("does not report full history as unpushed for fresh no-track Orqara worktrees", async () => {
    setupRemoteTrackingMain(repoDir, tempDir);
  ...
  it("resolves the Git common directory once when reading Orqara worktree facts", async () => {
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/HUSAM-07/orqara.git"], {
      cwd: repoDir,
    });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/HUSAM-07/orqara/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("does not attach an owner when the tracked remote is the same GitHub repository", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "git@github.com:HUSAM-07/orqara.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/HUSAM-07/orqara.git"], {
      cwd: repoDir,
    });
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    const github = createGitHubServiceRecordingPullRequestTargets({
      ...
      statusOverrides: {
        number: 345,
        url: "https://github.com/HUSAM-07/orqara/pull/345",
      },
    });
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/HUSAM-07/orqara/pull/999",
  ...
  it("does not reuse a PR status cache entry after HEAD changes on the same branch", async () => {
    ...
    github.getCurrentPullRequestStatus = async (options) => {
      ...
      return createPullRequestStatus({
        url: `https://github.com/HUSAM-07/orqara/pull/${requestedShas.length}`,
      });
    };
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/HUSAM-07/orqara/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Orqara worktrees (no heuristics)", async () => {
  ...
  it("excludes dirty working tree changes from Orqara worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Orqara worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
  ...
  it("falls back to plain git checkout status when Orqara worktree metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
