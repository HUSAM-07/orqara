...
export async function isPaseoOwnedWorktreeCwd(
  ...
  // The <hash>/<slug> prefix is Orqara-private — nothing else writes there — so the
...
export async function deletePaseoWorktree({
  ...
  if (worktreesRoot) {
    ...
    throw new Error("cwd or worktreesRoot is required to delete a Orqara worktree");
  }
  ...
  if (relativeWorktreePath === null || relativeWorktreePath === "") {
    throw new Error("Refusing to delete non-Orqara worktree");
  }
...
function normalizeRequiredBaseBranch(baseBranch: string): string {
  ...
  if (!normalizedBaseBranch) {
    throw new Error("Base branch is required when creating a Orqara worktree");
  }
  if (normalizedBaseBranch === "HEAD") {
    throw new Error("Base branch cannot be HEAD when creating a Orqara worktree");
  }
