...
const FDROID_ABI_VERSION_CODE_BLOCK = `// Orqara F-Droid single-ABI version codes
...
if (paseoArchitectures.size() == 1) {
    ...
    if (paseoAbiVersionCode == null) {
        throw new GradleException("Unsupported Orqara Android ABI: " + paseoAbi)
    }
...
function configureFdroidAppBuildGradle(contents) {
  ...
  if (!configuredContents.includes("// Orqara F-Droid single-ABI version codes")) {
