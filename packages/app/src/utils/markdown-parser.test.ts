...
describe("createMarkdownParser", () => {
  ...
  it("linkifies bare URLs only when asked", () => {
    expect(
      createMarkdownParser({ linkify: true }).render("see https://orqara.pages.dev now"),
    ).toContain('href="https://orqara.pages.dev"');
    expect(
      createMarkdownParser({ linkify: false }).render("see https://orqara.pages.dev now"),
    ).not.toContain("href");
  });
});
