...
describe("parseForgeRef", () => {
  it.each([
    [
      ...
      "https://github.com/HUSAM-07/orqara/pull/994/files?diff=split#discussion_r123",
      { kind: "change_request", number: 994 },
    ],
    [
      "GitHub issue",
      "https://github.com/HUSAM-07/orqara.git",
      "https://github.com/HUSAM-07/orqara/issues/456",
  ...
  it("canonicalizes a cloud SSH alias to its web host", () => {
    expect(
      parseForgeRef(
        "https://github.com/HUSAM-07/orqara/pull/994",
        "ssh://git@ssh.github.com/HUSAM-07/orqara.git",
      ),
  ...
  it("does not apply another forge's route grammar to a known cloud host", () => {
    expect(
      parseForgeRef(
        "https://github.com/HUSAM-07/orqara/pulls/31",
        "git@github.com:HUSAM-07/orqara.git",
      ),
    ).toBeNull();
    expect(
...
describe("extractForgeRefs", () => {
  ...
  it("returns no references without text or a valid remote", () => {
    expect(extractForgeRefs("", "git@github.com:HUSAM-07/orqara.git")).toEqual([]);
    expect(extractForgeRefs("https://github.com/HUSAM-07/orqara/pull/1", null)).toEqual([]);
  });
});
