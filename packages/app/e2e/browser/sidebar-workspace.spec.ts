...
async function withPaseoOwnedWorktree(
  ...
  try {
    ...
    if (!created.workspace) {
      throw new Error(created.error ?? "Failed to create Orqara-owned worktree");
    }
...
test.describe("Sidebar workspace list", () => {
  ...
  test("Orqara-owned worktree hover card shows the worktree directory name", async ({ page }) => {
