...
const SUPPORT_DESTINATION = /^https:\/\/github\.com\/HUSAM-07\/orqara\/discussions(?:[/?#]|$)/;
const GITHUB_ISSUE_DESTINATION =
  /^https:\/\/github\.com\/(?:HUSAM-07\/orqara\/issues\/new(?:\/choose)?(?:[/?#]|$)|login\?return_to=https%3A%2F%2Fgithub\.com%2FHUSAM-07%2Forqara%2Fissues%2Fnew$)/;
const RELEASES_DESTINATION = /^https:\/\/github\.com\/HUSAM-07\/orqara\/releases(?:[/?#]|$)/;
...
const APP_VERSION = /^Orqara\s*v\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/;
...
test("opens troubleshooting, support, and release destinations", async ({ page }) => {
  ...
  await test.step("opens support and release pages", async () => {
    await openHelpMenu(page);
    await expectExternalPage(page, "sidebar-help-discord", SUPPORT_DESTINATION);

    await openHelpMenu(page);
    await expectExternalPage(page, "sidebar-help-github", GITHUB_ISSUE_DESTINATION);

    await openHelpMenu(page);
    await expectExternalPage(page, "sidebar-help-changelog", RELEASES_DESTINATION);
  });
});
