...
const RELAY_DOCS_URL = "https://orqara.pages.dev/docs/security";
...
export function pairCommand(): Command {
  return addJsonOption(new Command("pair").description("Print the daemon pairing QR code and link"))
    .option("--home <path>", "Orqara home directory (default: ~/.paseo)")
...
async function resolveDaemonPairingOffer(
  ...
  try {
    ...
    if (serverInfo?.serverId.trim() !== expectedServerId) {
      throw new Error(
        "The reachable daemon belongs to a different Orqara home. Check --home or the daemon listen configuration.",
      );
    }
    if (serverInfo?.features?.daemonStatusRpc !== true) {
      throw new Error("Update the Orqara daemon before pairing from this command.");
    }
    ...
    if (!offer.relayEnabled && enableRelay) {
      if (serverInfo.features.relayConfig !== true) {
        throw new Error("Update the Orqara daemon before enabling relay from this command.");
      }
...
export async function confirmRelayPairing(): Promise<boolean> {
  log.message("Your connection is end-to-end encrypted. Orqara cannot read your code or messages.");
