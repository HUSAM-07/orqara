...
function printNextSteps(pairingUrl: string | null, paseoHome: string, richUi: boolean): void {
  ...
  const nextStepsLines = [
    pairingUrl
      ? "1. Open Orqara and scan the QR code above, or paste the pairing link."
      : "1. Open Orqara and connect to your daemon.",
    "2. Web app: https://app.paseo.sh",
    "3. Desktop app: https://github.com/HUSAM-07/orqara/releases/latest",
    "4. Docs: https://orqara.pages.dev/docs",
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  if (richUi) {
    renderNote(paseoHome, "Orqara home");
  }
  ...
  if (options.relay === false) {
    ...
    if (richUi) outro("Orqara daemon is running.");
    return;
  }
  ...
  if (!pairing.url) {
    ...
    printNextSteps(null, paseoHome, richUi);
    if (richUi) {
      outro("Orqara daemon is running.");
    }
    return;
  }
