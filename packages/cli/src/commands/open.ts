...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Orqara.app",
      path.join(homedir(), "Applications", "Orqara.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Orqara",
      "/opt/Orqara/Orqara",
      path.join(homedir(), "Applications", "Orqara.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Orqara", "Orqara.exe");
...
function launchDesktop(args: string[]): void {
  if (process.env.PASEO_DESKTOP_CLI === "1") {
    throw new Error("Cannot open Orqara Desktop while running in desktop CLI passthrough mode.");
  }

  const desktopApp = findDesktopApp();
  if (!desktopApp) {
    throw new Error(
      "Orqara desktop app not found. Install it from https://github.com/HUSAM-07/orqara/releases",
    );
  }
