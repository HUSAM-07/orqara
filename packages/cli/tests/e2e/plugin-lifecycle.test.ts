...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.match(incompatibleInstall.stderr, /requires Orqara >=999.0.0/);
    ...
    await git(gitDirectory, ["config", "user.name", "Orqara Tests"]);
    ...
    await git(gitDirectory, ["add", "-A"]);
    await git(gitDirectory, ["commit", "-m", "requires a future Orqara"]);
    const incompatibleUpdate = await context.paseo(["plugin", "update", "git-cli-e2e", "--json"]);
    assert.equal(incompatibleUpdate.exitCode, 1);
    assert.match(incompatibleUpdate.stderr, /requires Orqara >=999.0.0/);
    ...
    assert.match(incompatibleAdd.stderr, /requires Orqara >=999.0.0/);
    await assert.rejects(readFile(buildMarker), { code: "ENOENT" });
