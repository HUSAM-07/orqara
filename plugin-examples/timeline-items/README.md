...
calls return `undefined`, leaving Orqara's original timeline entry unchanged.

The transformer is a pure client contribution. It receives each source item and its streaming phase
while Orqara builds the render model, then returns plain plugin item objects. The renderer validates
`data` before Orqara mounts the component.
