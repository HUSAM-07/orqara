...
and SDK details behind Orqara's callback-based provider connection.

`server/vendor-edit.ts` shows the intended extension seam: a focused transformer converts one
vendor tool payload into Orqara's ordinary file-edit shape. It does not translate the whole event
stream.
